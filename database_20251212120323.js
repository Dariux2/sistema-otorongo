...
function insertSampleData() {
    ...
    db.get('SELECT COUNT(*) as count FROM users', async (err, row) => {
        ...
        if (row.count === 0) {
            ...
            // Insertar usuarios
            const users = [
                ['admin1', 'admin', adminPass, 'administrador', 'Dr. Carlos Mendoza', 'admin@otorongo.com'],
                ['medico1', 'medico', medicoPass, 'medico', 'Dra. Ana García', 'medico@otorongo.com'],
                ['recep1', 'recepcion', recepPass, 'recepcionista', 'María López', 'recepcion@otorongo.com']
            ];
            ...
            history.forEach(h => historyStmt.run(h));
            ...
            appointments.forEach(apt => appointmentStmt.run(apt));
            ...
            invoices.forEach(inv => invoiceStmt.run(inv));
